...
export const COLORS = {
    ...
    PiYGRev: ['#276419', '#4d9221', '#7fbc41', '#b8e186', '#e6f5d0', '#f7f7f7', '#fde0ef', '#f1b6da', '#de77ae', '#c51b7d', '#8e0152'],
    // Divergent
...
const METRIC_DATA: Record<string, Omit<MetricDef, 'category'>[]> = {
    ...
    'metrics.categories.safety': [
        ...
        },
        {
            ...
            icon: '🚗',
            format: (v, _min, _max, mode) => mode === 'all' ? (v || 0).toString() : Math.round((v || 0) * 100).toString(),
            unit: (mode, _) => mode === 'all' ? 'veh' : '%',
            scaleMethod: logarithmicScale, steps: 100, ignoreValues: [0, null],
            pallete: COLORS.WhiteToRed,
            sources: ['ansr', 'pmus']
        }
